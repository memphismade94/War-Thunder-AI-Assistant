...
USER_AGENT = "Warthog-Ground-RB-KnowledgeBot/0.2 (official-source research; respectful rate limit)"
...
def normalize(url):
    ...
    # Ignore tracking/query variants; the Wiki's content is represented by the canonical path.
...
def main():
    ...
    while queue and len(seen)<MAX_PAGES:
        ...
        text=clean(" ".join(p.parts)); title=clean(p.title) or url
        # Prefer a clean H1-like title when present in the document text.
        ...
        for href in p.links:
            ...
            if path.startswith(ALLOWED_PREFIXES):
                # Keep source pages first; deeper discovered pages inherit a slightly lower priority.
    ...
    manifest={"schema_version":2,"generated_at":datetime.now(timezone.utc).isoformat(),"pages":len(docs),"chunks":len(chunks),"failed_pages":failures,"sources_policy":"official-first","crawler_version":"0.2"}
